...
import Model, { ColumnNamesAliases, ColumnInput, PermittedColumns } from './model';
...
export default class SQLQuery {
  ...
  public order(input: SQLOrder): this {
    ...
    const [columns, joins] = input.reduce(
      ([columns, joins]: [string[], string[]], columnNameValue: string | [string, 'ASC' | 'DESC']): [string[], string[]] => {
        const [columnName, value] = columnNameValue instanceof Array ? columnNameValue : [columnNameValue, 'ASC'];
        this.model.validate([columnName]);
        const [addColumn, addJoins] = SQLQuery.parseColumn(this.model, columnName, value, '', 0, false);
  ...
  static select(model: Model, columnNames: ColumnNamesAliases = ['*']): SQLQuery {
    const [columns, joins] = this.parseSelectColumns(model, columnNames);
  ...
  static parseSelectColumns(model: Model, columnNames: ColumnNamesAliases): [string, string[]] {
    const [names, joins] = model.validate(columnNames).reduce(
      ([names, joins]: [string[], string[]], columnName: string | [string, string]): [string[], string[]] => {
        const [addName, addJoins] = this.parseColumn(model, columnName);
  ...
  static parseInsertUpdateColumns(
    model: Model, input: ColumnInput, enforceRequired = false, permitOnly?: PermittedColumns,
  ): [string, string, any[]] {

    const [names, values, params] = Object.entries(model.validate(input, false, enforceRequired, permitOnly)).reduce(
      (
        [names, values, params]: [string[], string[], any[]], [columnName, value]: [string, any], i: number,
      ): [string[], string[], any[]] => {
        return [names.concat(model.columns[columnName].name), values.concat(`$${i + 1}`), params.concat(value)];
      },
      [[], [], []]);
  ...
  static parseColumn(
    ...
    if (names.length > 1) {
      ...
      // check for left/right joins
      const joinMatches = names[0].match(/[?!]$/);
      const joinPrefix = joinMatches && (joinMatches[0] === '?' ? 'LEFT ' : 'RIGHT ') || '';

      // get join from model (last char removed from string in the event that a join type was detected above)
      const join = model.joins[joinMatches ? names[0].slice(0, -1) : names[0]];

      return this.parseColumn(
        join.foreignInstance!(),
        alias ? [names.slice(1).join('.'), alias] : names.slice(1).join('.'),
        value,
        operator,
        paramNext,
        parameterized,
        joins.concat(`${joinPrefix}JOIN ${join.foreignInstance!().table} ON ${model.table}.${model.columns[join.joinColumn].name} = ${join.foreignInstance!().table}.${join.foreignInstance!().columns[join.foreignJoinColumn].name}`),
      );

    }
    if (name === '*') return [`${model.table}.*`, joins, value, paramNext];

    const table = `${model.table}.${model.columns[name].name}`;

    if (value === undefined) return [alias ? `${table} AS ${alias.replace(/([^_])([A-Z])/g, '$1_$2')}` : table, joins, value, paramNext];
  ...
  static parseCondition(
    model: Model, condition: ColumnInput | SQLCondition, paramNext = 1, permitJoins = true, joins: string[] = [],
    ...
    return Object.entries(model.validate(condition, permitJoins)).reduce(
      ([queryCondition, params, joins, next]: [string, any[], string[], number], [column, valueOperator], i):
      [string, any[], string[], number] => {
...
export const and = function (...conditions: (ColumnInput | SQLCondition)[]): SQLCondition {
  return SQLQuery.createCondition('AND', ...conditions);
};

export const or = function (...conditions: (ColumnInput | SQLCondition)[]): SQLCondition {
